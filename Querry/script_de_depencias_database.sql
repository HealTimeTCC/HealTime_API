...
BEGIN
	...
	--	BEGIN
	--		DECLARE @COD_PRESCRICAO INT, @QTDE_HORARIOS INT;
	--		SELECT @COD_ANDAMENTO_INSERT =  PrescricaoMedicacaoId FROM inserted;
	--		IF(NOT EXISTS(SELECT 1 FROM ControleEstadoAndamentoMedicacao WHERE CodAndamentoMedicacao = @COD_ANDAMENTO_INSERT))
	--		BEGIN
	--			SELECT @QTDE_HORARIOS = COUNT(*) FROM AndamentoMedicacoes WHERE PrescricaoMedicacaoId = @COD_ANDAMENTO_INSERT;
